...
local function shockwave(center, radius, damage, ignoreModel)
	...
	effect.Size = Vector3.new(1, radius * 2, radius * 2)
	effect.CFrame = CFrame.new(center) * CFrame.Angles(0, 0, math.rad(90))
	effect.Material = Enum.Material.Neon
	effect.Transparency = 0.2
	effect.Color = Color3.fromRGB(170, 120, 255)
...
function BossService.CreateGuardian(position, parent, uniqueName)
	...
	model.Name = uniqueName or "CrystalGuardian"
	model:SetAttribute("Enemy", true)
	model:SetAttribute("BossId", "CrystalGuardian")
	model:SetAttribute("EnemyType", "CrystalGuardian")
	...
	body.Name = "Body"
	body.Size = Vector3.new(5, 7, 4)
	body.Position = position + Vector3.new(0, 6, 0)
	body.Material = Enum.Material.Neon
	body.Color = Color3.fromRGB(120, 80, 190)
	body.Parent = model
	local head = Instance.new("Part")
	head.Name = "Head"
	head.Shape = Enum.PartType.Ball
	head.Size = Vector3.new(3, 3, 3)
	head.Position = position + Vector3.new(0, 10.5, 0)
	head.Material = Enum.Material.Neon
	head.Color = Color3.fromRGB(180, 130, 255)
	head.Parent = model
	...
	task.spawn(function()
		while model.Parent and humanoid.Health > 0 do
			local nearestPlayer
			local nearestDistance = config.AggroRange
			...
			if nearestPlayer then
				...
				if targetRoot and targetHumanoid then
					if humanoid.Health <= config.Health * 0.5 then
	...
	humanoid.Died:Connect(function()
		...
		local player = creator and game:GetService("Players"):GetPlayerByUserId(creator)
		if player then
			...
			if profile then
				...
				InventoryService.AddItem(profile, config.Drop, 1)
				profile.Stats.BossesDefeated = (profile.Stats.BossesDefeated or 0) + 1
				PlayerService.Sync(player)
				player:SetAttribute("BossMessage", "Crystal Guardian defeated! +" .. config.XP .. " XP")
			end
		end
		task.delay(config.Respawn, function()
			if parent.Parent then BossService.CreateGuardian(position, parent, uniqueName) end
		end)
	end)
	return model
end
