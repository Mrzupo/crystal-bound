...
local function emitCombatEffect(crystalId, targetModel, ability)
	...
	effect.Color = crystalId == "EMBER" and Color3.fromRGB(255, 90, 35)
		or crystalId == "TIDE" and Color3.fromRGB(45, 150, 255)
		or Color3.fromRGB(170, 120, 255)
	effect.Parent = workspace
	TweenService:Create(effect, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
local function fireProgress(player, levelsGained, moneyChanged, inventoryChanged, mastery)
	...
	if mastery and remotes:FindFirstChild("CrystalMasteryChanged") then
		remotes.CrystalMasteryChanged:FireClient(player, mastery.Crystal, mastery.Level, mastery.XP)
	end
end
