...
local function emitCombatEffect(crystalId, targetModel, ability)
	...
	effect.Size = ability and Vector3.new(5, 5, 5) or Vector3.new(2, 2, 2); effect.CFrame = root.CFrame; effect.Transparency = 0.15
...
local function fireProgress(player, levelsGained, moneyChanged, inventoryChanged, mastery)
...
local function completeQuest(player, profile, questId, message)
	local definition = QuestSystem.GetDefinition(questId); if not definition or not QuestSystem.IsActive(profile, questId) then return false end
	...
	player:SetAttribute("QuestMessage", message or (definition.Name .. " complete!")); return true
end
...
local function giveLoot(profile, targetModel, crystalId)
	local enemyType = targetModel:GetAttribute("EnemyType"); local enemyConfig = enemyType and EnemyConfig.Get(enemyType) or nil
...
local function rewardDefeat(player, profile, targetModel, action, crystalId)
	...
	fireProgress(player, levelsGained or 0, true, true, { Crystal = crystalId, Level = masteryLevel, XP = masteryXP })
end
	...
	for _, enemy in ipairs(folder:GetChildren()) do
		if enemy ~= centerModel and enemy:IsA("Model") and enemy:GetAttribute("Enemy") == true then
			...
			if humanoid and humanoid.Health > 0 and root and (root.Position - centerRoot.Position).Magnitude <= 12 then
				humanoid:TakeDamage(math.max(1, damage * 0.45)); if humanoid.Health <= 0 and profile then rewardDefeat(player, profile, enemy, "Ability", "GALE") end
			end
		end
	end
end
...
function CombatService.HandleRequest(player, action, target)
	...
	local multiplier = math.max(0.1, tonumber(passive.DamageMultiplier) or 1) * mastery.DamageMultiplier; if action == "Ability" then multiplier *= mastery.AbilityDamageMultiplier end
	...
	local result = DamageService.ProcessDamage({ Attacker = player, Target = targetModel, Amount = damage, Range = config.Range, DamageType = "Crystal" })
