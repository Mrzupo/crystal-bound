...
local function emitCombatEffect(crystalId,targetModel,ability)
	local root=targetModel:FindFirstChild("HumanoidRootPart") or targetModel.PrimaryPart; if not root then return end
	local effect=Instance.new("Part"); effect.Name=ability and "CrystalAbilityEffect" or "CrystalHitEffect"; effect.Anchored=true; effect.CanCollide=false; effect.CanQuery=false; effect.CanTouch=false; effect.Shape=Enum.PartType.Ball; effect.Material=Enum.Material.Neon; effect.Size=ability and Vector3.new(5,5,5) or Vector3.new(2,2,2); effect.CFrame=root.CFrame; effect.Transparency=0.15; effect.Color=crystalId=="EMBER" and Color3.fromRGB(255,90,35) or crystalId=="TIDE" and Color3.fromRGB(45,150,255) or Color3.fromRGB(170,120,255); effect.Parent=workspace
	TweenService:Create(effect,TweenInfo.new(0.2),{Size=ability and Vector3.new(11,11,11) or Vector3.new(4,4,4),Transparency=1}):Play(); Debris:AddItem(effect,0.25)
end
local function fireProgress(player,levelsGained,mastery)
	local profile=PlayerService.GetProfile(player); if not profile then return end; PlayerService.Sync(player); local remotes=ReplicatedStorage:FindFirstChild("Remotes"); if not remotes then return end
...
local function giveLoot(profile,targetModel,crystalId)
	local enemyType=targetModel:GetAttribute("EnemyType"); local enemyConfig=enemyType and EnemyConfig.Get(enemyType) or nil; local itemId=enemyConfig and enemyConfig.Drop; if not itemId then itemId=({EMBER="EmberShard",TIDE="TidePearl",GALE="GaleFeather"})[crystalId] end; if itemId then return InventoryService.AddItem(profile,itemId,1) end; return 0
end
...
local function applyGaleSplash(player,centerModel,damage)
	local centerRoot=centerModel:FindFirstChild("HumanoidRootPart") or centerModel.PrimaryPart; if not centerRoot then return end; local folder=workspace:FindFirstChild("NPCs"); if not folder then return end; local profile=PlayerService.GetProfile(player)
	for _,enemy in ipairs(folder:GetChildren()) do if enemy~=centerModel and enemy:IsA("Model") and enemy:GetAttribute("Enemy")==true and enemy:GetAttribute("BossId")==nil then local humanoid=enemy:FindFirstChildOfClass("Humanoid"); local root=enemy:FindFirstChild("HumanoidRootPart") or enemy.PrimaryPart; if humanoid and humanoid.Health>0 and root and (root.Position-centerRoot.Position).Magnitude<=12 then humanoid:TakeDamage(math.max(1,damage*0.45)); if humanoid.Health<=0 and profile then rewardDefeat(player,profile,enemy,"Ability","GALE") end end end end
end
local function applyAbilitySpecial(player,profile,crystalId,targetModel,abilityDamage)
	if crystalId=="TIDE" then local character=player.Character; local humanoid=character and character:FindFirstChildOfClass("Humanoid"); if humanoid and humanoid.Health>0 then humanoid.Health=math.min(humanoid.MaxHealth,humanoid.Health+30); player:SetAttribute("CrystalMessage","Tidal Pulse restored health.") end elseif crystalId=="GALE" then applyGaleSplash(player,targetModel,abilityDamage) end
end
