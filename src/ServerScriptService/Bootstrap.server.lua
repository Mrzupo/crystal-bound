...
local function spawnQuestGiver(npcs)
	spawnSimpleNPC(npcs,"CrystalKeeper",Vector3.new(12,3,-2),"Crystal Keeper","Quest",function(player)
		...
		if not QuestSystem.IsActive(profile,"FIRST_FIGHT") and not QuestSystem.IsCompleted(profile,"FIRST_FIGHT") then questToStart="FIRST_FIGHT" elseif not QuestSystem.IsActive(profile,"CRYSTAL_POWER") and not QuestSystem.IsCompleted(profile,"CRYSTAL_POWER") then questToStart="CRYSTAL_POWER" elseif profile.Level>=3 and not QuestSystem.IsActive(profile,"HUNT_EMBERLINGS") and not QuestSystem.IsCompleted(profile,"HUNT_EMBERLINGS") then questToStart="HUNT_EMBERLINGS" elseif profile.Level>=6 and not QuestSystem.IsActive(profile,"TIDE_EXPEDITION") and not QuestSystem.IsCompleted(profile,"TIDE_EXPEDITION") then questToStart="TIDE_EXPEDITION" elseif profile.Level>=10 and not QuestSystem.IsActive(profile,"WIND_TRIAL") and not QuestSystem.IsCompleted(profile,"WIND_TRIAL") then questToStart="WIND_TRIAL" elseif profile.Level>=15 and not QuestSystem.IsActive(profile,"GUARDIAN_TRIAL") and not QuestSystem.IsCompleted(profile,"GUARDIAN_TRIAL") then questToStart="GUARDIAN_TRIAL" elseif profile.Level>=18 and not QuestSystem.IsActive(profile,"GOLEM_HUNT") and not QuestSystem.IsCompleted(profile,"GOLEM_HUNT") then questToStart="GOLEM_HUNT" elseif profile.Level>=19 and not QuestSystem.IsActive(profile,"BAT_HUNT") and not QuestSystem.IsCompleted(profile,"BAT_HUNT") then questToStart="BAT_HUNT" end
...
local function spawnTrader(npcs) spawnSimpleNPC(npcs,"MaterialTrader",Vector3.new(20,3,10),"Material Trader","Sell Loot",function(player) player:SetAttribute("ShopMessage","Sell materials with 4/5/6/7/8. Upgrade at the Crystal Keeper with U.") end) end
