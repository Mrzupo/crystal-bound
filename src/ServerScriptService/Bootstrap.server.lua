...
remotes.InventoryRequest.OnServerEvent:Connect(function(player, action, itemId, amount)
	...
	if action == "Sell" and type(itemId) == "string" then
		...
		local trader = Workspace:FindFirstChild("NPCs") and Workspace.NPCs:FindFirstChild("MaterialTrader"); local traderRoot = trader and (trader.PrimaryPart or trader:FindFirstChild("Torso"))
...
remotes.GetPlayerData.OnServerInvoke = function(player)
	local profile = PlayerService.GetProfile(player); if not profile then return nil end
...
remotes.GetQuestData.OnServerInvoke = function(player)
	local profile = PlayerService.GetProfile(player); if not profile then return { Active = {}, Completed = {}, Progress = {}, Definitions = QuestSystem.GetDefinitions() } end
...
local function ensureWorldFolders()
	for _, name in ipairs({ "NPCs", "Islands", "Spawn" }) do local folder = Workspace:FindFirstChild(name); if not folder then folder = Instance.new("Folder"); folder.Name = name; folder.Parent = Workspace end end
...
local function createPortal(island, name, fromPosition, destination, requiredLevel)
	...
	portal.Touched:Connect(function(hit)
		...
		local profile = PlayerService.GetProfile(player); if not profile or profile.Level < (requiredLevel or 1) then player:SetAttribute("PortalMessage", string.format("Reach level %d to use this portal", requiredLevel or 1)); return end
...
local function spawnQuestGiver(npcs)
	spawnSimpleNPC(npcs, "CrystalKeeper", Vector3.new(12, 3, -2), "Crystal Keeper", "Quest", function(player)
		...
		elseif profile.Level >= 18 and not QuestSystem.IsActive(profile, "ANCIENT_HUNT") and not QuestSystem.IsCompleted(profile, "ANCIENT_HUNT") then questToStart = "ANCIENT_HUNT" end
...
local function spawnTrader(npcs)
	spawnSimpleNPC(npcs, "MaterialTrader", Vector3.new(20, 3, 10), "Material Trader", "Sell Loot", function(player) player:SetAttribute("ShopMessage", "Sell materials with 4/5/6. Upgrade the equipped crystal with U.") end)
end
...
local ancientIsland = createIsland(islands, "AncientRuins", Vector3.new(500, 0, 0), Vector3.new(130, 2, 130))
...
createPortal(windIsland, "AncientPortal", Vector3.new(390, 5, 0), Vector3.new(440, 4, 0), 15)
createPortal(ancientIsland, "WindReturnPortal", Vector3.new(440, 5, 50), Vector3.new(380, 4, 0), 1)
...
spawnEnemy(npcs, "CrystalBat", Vector3.new(475, 1, -20), "CrystalBatA")
spawnEnemy(npcs, "CrystalBat", Vector3.new(525, 1, -10), "CrystalBatB")
spawnEnemy(npcs, "AncientGolem", Vector3.new(500, 1, 25), "AncientGolemA")
