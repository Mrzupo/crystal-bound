...
remotes.QuestRequest.OnServerEvent:Connect(function(player, action, questId) local profile = PlayerService.GetProfile(player); if profile and action == "Start" and type(questId) == "string" then QuestService.Start(player, profile, questId) end end)
remotes.InventoryRequest.OnServerEvent:Connect(function(player, action, itemId, amount)
	...
	if action == "Sell" and type(itemId) == "string" then
		local character = player.Character; local root = character and character:FindFirstChild("HumanoidRootPart"); local trader = Workspace:FindFirstChild("NPCs") and Workspace.NPCs:FindFirstChild("MaterialTrader"); local traderRoot = trader and (trader.PrimaryPart or trader:FindFirstChild("Torso"))
...
remotes.CrystalChanged.OnServerEvent:Connect(function(player, crystalId)
	...
	local profile = PlayerService.GetProfile(player); if not profile then return end
	local requiredLevel = crystalRequirements[crystalId] or math.huge; if profile.Level < requiredLevel then player:SetAttribute("CrystalMessage", string.format("%s unlocks at level %d", crystalId, requiredLevel)); return end
...
local function spawnQuestGiver(npcs)
	spawnSimpleNPC(npcs, "CrystalKeeper", Vector3.new(12, 3, -2), "Crystal Keeper", "Quest", function(player)
		local profile = PlayerService.GetProfile(player); if not profile then return end; local questToStart
		...
		elseif profile.Level >= 15 and not QuestSystem.IsActive(profile, "GUARDIAN_TRIAL") and not QuestSystem.IsCompleted(profile, "GUARDIAN_TRIAL") then questToStart = "GUARDIAN_TRIAL"
		elseif profile.Level >= 18 and not QuestSystem.IsActive(profile, "GOLEM_HUNT") and not QuestSystem.IsCompleted(profile, "GOLEM_HUNT") then questToStart = "GOLEM_HUNT"
		elseif profile.Level >= 19 and not QuestSystem.IsActive(profile, "BAT_HUNT") and not QuestSystem.IsCompleted(profile, "BAT_HUNT") then questToStart = "BAT_HUNT" end
...
local function spawnTrader(npcs) spawnSimpleNPC(npcs, "MaterialTrader", Vector3.new(20, 3, 10), "Material Trader", "Sell Loot", function(player) player:SetAttribute("ShopMessage", "Sell materials with 4/5/6. Upgrade the equipped crystal with U.") end) end
...
local starterIsland = createIsland(islands, "StarterIsland", Vector3.new(0, 0, 0), Vector3.new(120, 2, 120)); local tideIsland = createIsland(islands, "TideIsland", Vector3.new(170, 0, 0), Vector3.new(100, 2, 100)); local windIsland = createIsland(islands, "WindIsland", Vector3.new(330, 0, 0), Vector3.new(110, 2, 110)); local ancientIsland = createIsland(islands, "AncientRuins", Vector3.new(500, 0, 0), Vector3.new(130, 2, 130))
...
spawnEnemy(npcs, "TrainingDummy", Vector3.new(0, 1, -12), "TrainingDummy"); spawnEnemy(npcs, "Emberling", Vector3.new(30, 1, -18), "EmberlingA"); spawnEnemy(npcs, "Emberling", Vector3.new(-30, 1, -18), "EmberlingB"); spawnEnemy(npcs, "Tidecrawler", Vector3.new(160, 1, -12), "TidecrawlerA"); spawnEnemy(npcs, "Tidecrawler", Vector3.new(190, 1, 15), "TidecrawlerB"); spawnEnemy(npcs, "Galewisp", Vector3.new(300, 1, -18), "GalewispA"); spawnEnemy(npcs, "Galewisp", Vector3.new(340, 1, 20), "GalewispB"); spawnEnemy(npcs, "Galewisp", Vector3.new(370, 1, -10), "GalewispC"); spawnEnemy(npcs, "CrystalBat", Vector3.new(475, 1, -20), "CrystalBatA"); spawnEnemy(npcs, "CrystalBat", Vector3.new(525, 1, -10), "CrystalBatB"); spawnEnemy(npcs, "AncientGolem", Vector3.new(500, 1, 25), "AncientGolemA")
if not npcs:FindFirstChild("CrystalGuardian") then BossService.CreateGuardian(Vector3.new(330, 1, 0), npcs, "CrystalGuardian") end
local function initializePlayer(player) local profile = PlayerService.Load(player); if profile and not QuestSystem.IsActive(profile, "FIRST_FIGHT") and not QuestSystem.IsCompleted(profile, "FIRST_FIGHT") then QuestService.Start(player, profile, "FIRST_FIGHT") end end
...
task.spawn(function() while true do task.wait(AUTOSAVE_INTERVAL); for _, player in ipairs(Players:GetPlayers()) do PlayerService.Save(player) end end end); game:BindToClose(function() for _, player in ipairs(Players:GetPlayers()) do PlayerService.Save(player) end end)
