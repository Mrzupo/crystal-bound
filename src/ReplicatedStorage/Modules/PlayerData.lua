...
function PlayerData.Reconcile(data)
	local defaults = PlayerData.new(); data = type(data) == "table" and data or {}
	local function merge(target, fallback)
		for key, value in pairs(fallback) do
			if target[key] == nil then target[key] = clone(value)
			elseif type(value) == "table" and type(target[key]) == "table" then merge(target[key], value) end
		end
	end
	merge(data, defaults); data.Version = defaults.Version; return data
end
return PlayerData
