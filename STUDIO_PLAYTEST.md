...
- Controlled race test: trigger two profile-load attempts for the same UserId in rapid succession through a test harness.
- Expected: only the current load remains authoritative; a superseded successful load releases only its own session token and cannot release the newer session lock.
- Controlled leave/shutdown race: remove the Player while profile initialization is waiting.
- Expected: no profile is installed after removal, and the load-owned session token is released.
